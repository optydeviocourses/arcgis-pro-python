import os, sys, json, urllib3, httplib2, urlparse2, Products.MIMETools, mimetypes
...
def generate_token(baseurl, username, password):
    tokenUrl = urlparse2.urljoin(baseurl, 'sharing/generateToken')
    postdata = { 'username': username, 'password': password,
               'client': 'requestip', 'expiration': 60, 'f': 'json' }
    encoded_postdata = urllib3.urlencode(postdata)
    opener = urllib3.build_opener()
    try: resp = opener.open(tokenUrl, data=encoded_postdata)
    except urllib3.HTTPError as e: raise Exception('Unable to connect to Portal, please check the url: {} {}'.format(e.code, e.reason))
...
def _encode_multipart_formdata(fields, files):
    boundary = Products.MIMETools.choose_boundary()
    buf = StringIO()
...
def _postmultipart(host, selector, fields, files, ssl):
    ...
    if ssl: h = httplib2.HTTPSConnection(host)
    else: h = httplib2.HTTPConnection(host)
...
def addSDItem(baseurl, user, token, title, file, folder = ''):
    ...
    url = urlparse2.urljoin(baseurl, addUrl)
    ...
    parsed_url = urlparse2.urlparse(url)
...
def publishTiles(baseurl, user, token, itemid):
    publishUrl = urlparse2.urljoin(baseurl, 'sharing/rest/content/users/{}/publish'.format(user))
    ...
    query_string = urllib3.urlencode(query_dict)
    print('Publishing tile service from item..')
    response = urllib3.urlopen(publishUrl, query_string)
